...
abstract class Package implements ActiveRecordInterface
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\Map\\PackageTableMap';
    ...
    /**
     * Initializes internal state of Base\Package object.
     */
    ...
    public function setReference($v)
    {
        ...
        if ($this->reference !== $v) {
            $this->reference = $v;
            $this->modifiedColumns[PackageTableMap::COL_REFERENCE] = true;
        }
    ...
    public function setSenddate($v)
    {
        ...
        if ($this->senddate !== null || $dt !== null) {
            if ($this->senddate === null || $dt === null || $dt->format("Y-m-d H:i:s.u") !== $this->senddate->format("Y-m-d H:i:s.u")) {
                ...
                $this->modifiedColumns[PackageTableMap::COL_SENDDATE] = true;
            }
    ...
    public function setUserid($v)
    {
        ...
        if ($this->userid !== $v) {
            $this->userid = $v;
            $this->modifiedColumns[PackageTableMap::COL_USERID] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : PackageTableMap::translateFieldName('Reference', TableMap::TYPE_PHPNAME, $indexType)];
            $this->reference = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : PackageTableMap::translateFieldName('Senddate', TableMap::TYPE_PHPNAME, $indexType)];
            if ($col === '0000-00-00 00:00:00') {
                $col = null;
            }
            $this->senddate = (null !== $col) ? PropelDateTime::newInstance($col, null, 'DateTime') : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 3 + $startcol : PackageTableMap::translateFieldName('Userid', TableMap::TYPE_PHPNAME, $indexType)];
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(PackageTableMap::COL_REFERENCE)) {
            $modifiedColumns[':p' . $index++]  = 'reference';
        }
        if ($this->isColumnModified(PackageTableMap::COL_SENDDATE)) {
            $modifiedColumns[':p' . $index++]  = 'sendDate';
        }
        if ($this->isColumnModified(PackageTableMap::COL_USERID)) {
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(PackageTableMap::COL_REFERENCE)) {
            $criteria->add(PackageTableMap::COL_REFERENCE, $this->reference);
        }
        if ($this->isColumnModified(PackageTableMap::COL_SENDDATE)) {
            $criteria->add(PackageTableMap::COL_SENDDATE, $this->senddate);
        }
        if ($this->isColumnModified(PackageTableMap::COL_USERID)) {
            $criteria->add(PackageTableMap::COL_USERID, $this->userid);
        }
    ...
    public function setItemInPackages(Collection $itemInPackages, ConnectionInterface $con = null)
    {
        ...
        foreach ($itemInPackagesToDelete as $itemInPackageRemoved) {
            $itemInPackageRemoved->setPackage(null);
        }
     ...
     * @return ObjectCollection|ChildItemInPackage[] List of ChildItemInPackage objects
     */
    public function getItemInPackagesJoinItem(Criteria $criteria = null, ConnectionInterface $con = null, $joinBehavior = Criteria::LEFT_JOIN)
    {
        $query = ChildItemInPackageQuery::create(null, $criteria);
