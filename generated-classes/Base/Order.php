...
abstract class Order implements ActiveRecordInterface
{
    ...
    public function setReference($v)
    {
        ...
        if ($this->reference !== $v) {
            $this->reference = $v;
            $this->modifiedColumns[OrderTableMap::COL_REFERENCE] = true;
        }
    ...
    public function setSenddate($v)
    {
        ...
        if ($this->senddate !== null || $dt !== null) {
            if ($this->senddate === null || $dt === null || $dt->format("Y-m-d H:i:s.u") !== $this->senddate->format("Y-m-d H:i:s.u")) {
                ...
                $this->modifiedColumns[OrderTableMap::COL_SENDDATE] = true;
            }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : OrderTableMap::translateFieldName('Reference', TableMap::TYPE_PHPNAME, $indexType)];
            $this->reference = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : OrderTableMap::translateFieldName('Senddate', TableMap::TYPE_PHPNAME, $indexType)];
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(OrderTableMap::COL_REFERENCE)) {
            $modifiedColumns[':p' . $index++]  = 'reference';
        }
        if ($this->isColumnModified(OrderTableMap::COL_SENDDATE)) {
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(OrderTableMap::COL_REFERENCE)) {
            $criteria->add(OrderTableMap::COL_REFERENCE, $this->reference);
        }
        if ($this->isColumnModified(OrderTableMap::COL_SENDDATE)) {
            $criteria->add(OrderTableMap::COL_SENDDATE, $this->senddate);
        }
        if ($this->isColumnModified(OrderTableMap::COL_USERID)) {
