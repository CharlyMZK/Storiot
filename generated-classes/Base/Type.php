 ...
 */
abstract class Type implements ActiveRecordInterface
{
    /**
     * TableMap class name
     */
    const TABLE_MAP = '\\Map\\TypeTableMap';
    ...
    /**
     * Initializes internal state of Base\Type object.
     */
    ...
    /**
     * Compares this with another <code>Type</code> instance.  If
     * <code>obj</code> is an instance of <code>Type</code>, delegates to
     * <code>equals(Type)</code>.  Otherwise, returns <code>false</code>.
     *
     ...
     *
     * @return $this|Type The current object, for fluid interface
     */
     ...
     * @return $this|\Type The current object (for fluent API support)
     */
    public function setId($v)
    {
        ...
        if ($this->id !== $v) {
            $this->id = $v;
            $this->modifiedColumns[TypeTableMap::COL_ID] = true;
        }
    ...
    public function setName($v)
    {
        ...
        if ($this->name !== $v) {
            $this->name = $v;
            $this->modifiedColumns[TypeTableMap::COL_NAME] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 0 + $startcol : TypeTableMap::translateFieldName('Id', TableMap::TYPE_PHPNAME, $indexType)];
            $this->id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : TypeTableMap::translateFieldName('Name', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            return $startcol + 2; // 2 = TypeTableMap::NUM_HYDRATE_COLUMNS.

        } catch (Exception $e) {
            throw new PropelException(sprintf('Error populating %s object', '\\Type'), 0, $e);
        }
    }
    ...
    public function reload($deep = false, ConnectionInterface $con = null)
    {
        ...
        $dataFetcher = ChildTypeQuery::create(null, $this->buildPkeyCriteria())->setFormatter(ModelCriteria::FORMAT_STATEMENT)->find($con);
    ...
    public function delete(ConnectionInterface $con = null)
    {
        ...
        $con->transaction(function () use ($con) {
            $deleteQuery = ChildTypeQuery::create()
    ...
    public function save(ConnectionInterface $con = null)
    {
        ...
        return $con->transaction(function () use ($con) {
            ...
            if ($ret) {
                ...
                $this->postSave($con);
                TypeTableMap::addInstanceToPool($this);
            } else {
                $affectedRows = 0;
            }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(TypeTableMap::COL_ID)) {
            $modifiedColumns[':p' . $index++]  = 'id';
        }
        if ($this->isColumnModified(TypeTableMap::COL_NAME)) {
            $modifiedColumns[':p' . $index++]  = 'name';
        }

        $sql = sprintf(
            'INSERT INTO type (%s) VALUES (%s)',
    ...
    {
        $pos = TypeTableMap::translateFieldName($name, $type, TableMap::TYPE_NUM);
    ...
    public function toArray($keyType = TableMap::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false)
    {
        ...
        if (isset($alreadyDumpedObjects['Type'][$this->hashCode()])) {
            return '*RECURSION*';
        }
        $alreadyDumpedObjects['Type'][$this->hashCode()] = true;
        $keys = TypeTableMap::getFieldNames($keyType);
        $result = array(
            $keys[0] => $this->getId(),
            $keys[1] => $this->getName(),
        );
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(TypeTableMap::COL_ID)) {
            $criteria->add(TypeTableMap::COL_ID, $this->id);
        }
        if ($this->isColumnModified(TypeTableMap::COL_NAME)) {
            $criteria->add(TypeTableMap::COL_NAME, $this->name);
        }
    ...
    {
        $criteria = ChildTypeQuery::create();
        $criteria->add(TypeTableMap::COL_ID, $this->id);
     ...
     * If this ChildType is new, it will return
     ...
     * @param      ConnectionInterface $con Optional connection object
     * @return $this|ChildType The current object (for fluent API support)
     */
     ...
     * @return ObjectCollection|ChildItemType[] List of ChildItemType objects
     */
    public function getItemTypesJoinItem(Criteria $criteria = null, ConnectionInterface $con = null, $joinBehavior = Criteria::LEFT_JOIN)
    {
        $query = ChildItemTypeQuery::create(null, $criteria);
    ...
    {
        return (string) $this->exportTo(TypeTableMap::DEFAULT_STRING_FORMAT);
    }
