...
abstract class Contact implements ActiveRecordInterface
{
    ...
    public function setCategory($v)
    {
        ...
        if ($this->category !== $v) {
            $this->category = $v;
            $this->modifiedColumns[ContactTableMap::COL_CATEGORY] = true;
        }
    ...
    public function setSubject($v)
    {
        ...
        if ($this->subject !== $v) {
            $this->subject = $v;
            $this->modifiedColumns[ContactTableMap::COL_SUBJECT] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : ContactTableMap::translateFieldName('Category', TableMap::TYPE_PHPNAME, $indexType)];
            $this->category = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 3 + $startcol : ContactTableMap::translateFieldName('Subject', TableMap::TYPE_PHPNAME, $indexType)];
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(ContactTableMap::COL_CATEGORY)) {
            $modifiedColumns[':p' . $index++]  = 'category';
        }
        if ($this->isColumnModified(ContactTableMap::COL_SUBJECT)) {
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(ContactTableMap::COL_CATEGORY)) {
            $criteria->add(ContactTableMap::COL_CATEGORY, $this->category);
        }
        if ($this->isColumnModified(ContactTableMap::COL_SUBJECT)) {
            $criteria->add(ContactTableMap::COL_SUBJECT, $this->subject);
        }
        if ($this->isColumnModified(ContactTableMap::COL_MESSAGE)) {
